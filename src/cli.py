...
def f1_listener_start(port: int, no_serial: bool, mqtt: bool, mqtt_broker: str):
    ...
    def _on_telemetry(header, data):
        click.echo(f"\rsteer: {data.steer:+.2f}", nl=False)

        if ser and ser.is_open:
            steer_byte = int((data.steer + 1.0) * 127.5)
            steer_byte = max(0, min(255, steer_byte))
            cmd = f"RC:{steer_byte},128,0\n"
...
def _read_diagnostics(port: str, baud: int = 115200, timeout_s: int = 30):
